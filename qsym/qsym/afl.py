...
class AFLExecutor(object):
    def __init__(self, cmd, output, afl, name, vagrant_aflPath = None, filename=None, mail=None, asan_bin=None):
	self.vagrant_aflPath = vagrant_aflPath
	...
	if not (self.vagrant_aflPath is None):
	    cmd, afl_path, qemu_mode = self.parse_fuzzer_stats_vagrant()
	else:	
    ...
    def parse_fuzzer_stats_vagrant(self):

        cmd = get_afl_cmd(self.vagrant_aflPath)
