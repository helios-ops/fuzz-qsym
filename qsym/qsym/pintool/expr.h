...
class Expr : public DependencyNode {
  public:
    ...
    RangeSet* getSignedRangeSet() const 
    { 
	return getRangeSet(false); 
    }

    RangeSet* getUnsignedRangeSet() const 
    { 
	return getRangeSet(true); 
    }
  ...
  protected:
    ...
    // deps_ : 数据的引入从 ReadExpr 的读取索引开始。其实维护着当前的符号表达式所依赖的输入字节偏移位置
    DepSet* deps_;
